...
def processa_site(nome_site, sessao=None, assunto=None, limite_artigos=100):
    if nome_site == 'tercalivre':
        if sessao and assunto:
            ...
            exporta_json(scraped_site, f'{nome_site}-{sessao}-{assunto}-sites.json')
            scraped_pages = scrap_tercalivre_conteudo(scraped_site)
            exporta_json(scraped_pages, f'{nome_site}-{sessao}-{assunto}.json') 
    elif nome_site == '247':
        if sessao and assunto:
        ...
        if sessao and assunto:
            scraped_site = scrap_conexaopolitica(f'https://conexaopolitica.com.br/{sessao}/{assunto}', f'https://conexaopolitica.com.br/{sessao}/{assunto}/page', limite_artigos)
            exporta_json(scraped_site, f'{nome_site}-{sessao}-{assunto}-sites.json')
        ...
        if sessao:
            scraped_site = scrap_senso(f'https://sensoincomum.org/{sessao}', f'https://sensoincomum.org/category/{sessao}/page/', limite_artigos)
            exporta_json(scraped_site, f'{nome_site}-{sessao}-sites.json')
            scraped_pages = scrap_senso_conteudo(scraped_site)
            exporta_json(scraped_pages, f'{nome_site}-{sessao}.json')   
    elif nome_site == 'dcm':
